<#
    ...
    Updates
		...
		1.0.2 - 3/25/2018 - v-pemic
            -Changed search values from 'Lockheed' to 'Leidos' and 'Accenture'
			
        1.0.1 - 3/25/2018 - v-pemic
            -Fixed KPI issue for approved tickets

  		1.0.0 - 3/25/2018 - v-pemic
			-Script created to combine RDTask Triage looper and RDTask looper
...
$host.ui.RawUI.WindowTitle = "RDTask - Version: $version"
...
$TFSToApproveQuery = "SELECT [System.Id]
            ...
            WHERE 
                ...
                [Microsoft.RD.KeywordSearch] not contains 'SCRAP' AND
                [Microsoft.RD.KeywordSearch] not contains 'FF_SCRAP'
...
Function Get-RTOTasks()
{
	...
	{
		$WorkItemStore = Connect-AzureTfs -TfsUri https://vstfrdext.partners.extranet.microsoft.com:8443
		$error.Clear()
	}
    ...
    $Appitems | %{
		...
		$rev | %{$revdate = $_.fields['Changed Date'].Value
			...
			if (($AssignedTo -match "Leidos" -OR $AssignedTo -match "Accenture") -AND $PStatus -eq "Blocked"){
				if($revdate -gt $cdate){$cdate = $revdate}}
			if (($AssignedTo -match "Leidos" -OR $AssignedTo -match "Accenture") -AND $Pstate -eq "Resolved"){
				if($revdate -gt $cdate){$cdate = $revdate}}
			}
    ...
    $ValidTaskList | %{
        ...
        $OutputEntry | Add-Member -MemberType noteProperty -Name "State" -Value "Approved"
