...
$getLatestDepTTS = Get-ChildItem $ScriptDirectory | Where-Object {$_.Name -match "Fairfax-Deployment-TTS"} | Sort-Object $_.Name -Descending
if(!($getLatestDepTTS)){
    $getLatestDepTTS = Get-ChildItem "$ScriptDirectory\archive" | Where-Object {$_.Name -match "Fairfax-Deployment-TTS"} | Sort-Object $_.Name -Descending
    $DepTTSname = $getLatestDepTTS[0].Name
    $DepTTS = "archive\$DepTTSname"
	}
else{
	$DepTTS = $getLatestDepTTS[0].Name}
...
$Windows = @(
    ...
    # Upper Left
	@{ "X" = 0;  "Y" = 0; "X_SIZE" = 945;  "Y_SIZE" = 500; "Script" = ".\$DepTTS" }
