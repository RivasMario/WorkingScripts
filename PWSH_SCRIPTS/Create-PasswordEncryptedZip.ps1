...
{ 
   $THIS_SCRIPTS_DIRECTORY = Split-Path -Parent -Path $MyInvocation.MyCommand.Definition 
}
else
{ 
   $THIS_SCRIPTS_DIRECTORY = Split-Path -Parent -Path ([Environment]::GetCommandLineArgs()[0]) 
   if (!$THIS_SCRIPTS_DIRECTORY){ $ScriptPath = "." } 
}
